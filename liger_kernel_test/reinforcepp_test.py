...
from liger_kernel.chunked_loss import LigerFusedLinearReinforceppLoss

# @pytest.mark.parametrize("use_ref_model", [False, True])
def test_reinforcepp_loss_forward_backward(use_ref_model):
    ...
    original_sft_target = torch.nn.functional.softmax(logits, dim=-1)
    ...
    # Instantiate the reinforce++ loss module
    reinforcepp_loss_fn = LigerFusedLinearReinforceppLoss(
        ignore_index=-100,
        beta=0.1,
        compiled=True,       
    ...
    # Forward pass
    loss = reinforcepp_loss_fn(
        _input=_input,
        lin_weight=lin_weight,
        bias=bias,
        target=target,
        sft_target = original_sft_target, 
...
test_reinforcepp_loss_forward_backward(use_ref_model=True)
